        ...
        <div class="card bg-pattern">
            ...
            <div class="card-body p-4">
                ...
                <div class="mt-3 text-center">
                    ...
                    <h3>{{ $data['title'] }}</h3>
                    <p class="text-muted font-14 mt-2"> {{ $data['message'] }} </p>
                    @if( array_key_exists('action', $data) && $data['action'])
                    <a href="{{ route('login') }}" class="btn btn-primary d-grid secondary-bg-color mt-3">@if(isset($data['btnTxt'])) {{$data['btnTxt']}} @else Back to Log in @endif</a>
                    @endif
                </div>
